  ...
  <div class="vchat-Detail">
    ...
    <el-carousel trigger="click" height="200px" arrow="never" indicator-position="none">
      <el-carousel-item v-for="item in 1" :key="item">
        <a class="DetailImage-a" :style="{backgroundImage: 'url('+ IMG_URL + groupInfo.img +')'}"></a>
      </el-carousel-item>
      <div class="DetailImage-bg">
        <p class="title">{{groupInfo.title}}</p>
        <p>{{groupInfo.code}}</p>
    ...
    getGroupInfo() {
      ...
      api.getGroupInfo(params).then(r => {
        if (r.code === 0) {
          ...
          this.applyFlag = this.groupUsers.filter(
            v => v.userName === this.user.name
          ).length;
          this.managers = this.groupUsers.filter(
            v => v.holder === 1 || v.manager === 1
          );
          this.holderId = this.groupUsers.filter(v => v.holder === 1)[0].userId[
            "_id"
          ];
        }
      });
