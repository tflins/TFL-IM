  ...
  <div class="vchat-ownGroup vchat-noUser">
    ...
    <div class="vchat-ownGroup-container" v-if="Groups.length" v-loading="loadingWait">
      <div class="vchat-linkman-container" :class="{active: showList.indexOf('set') > -1}">
        ...
        <ul class="vchat-linkman-list">
          <li
            v-for="v in mySetGroups"
            :key="v._id"
            @click="goGroupDetail(v.groupId._id)"
    ...
    <v-nodata v-else>
      <p class="vchat-no-have">
        还没有加入群聊哦，去
        <router-link to="/main/personalMain/group/search">查找</router-link>， 去
...
export default {
  ...
  computed: {
    ...
    addOrDel() {
      return this.conversationsList.filter(v => v.id === this.currGroup._id)
        .length;
    }
  },
  methods: {
