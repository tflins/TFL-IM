  ...
  <div class="vchat-todo">
    ...
    <full-calendar :events="fcEvents" locale="zh-cn" lang="zh" @dayClick="dayClick">
      <template slot="fc-event-card" slot-scope="p">
        <el-popover placement="bottom" :title="p.event.title" width="200" trigger="click">
          <p class="todoInfo">时间：{{formatTime(p.event.start)}} 至 {{formatTime(p.event.end)}}</p>
          <p class="todoInfo">地点：{{p.event.address}}</p>
          <p class="todoInfo">内容：{{p.event.content}}</p>
