...
const createGroup = (params, callback) => {
  ...
  function fineOneAccountBase(createfun) {
    ...
    baseList.accountBase.findOneAndUpdate(
      { type: '2', status: '0', random: { $gte: rand } },
      { status: '1' },
      (err, doc) => {
        if (err) {
          console.log(err)
...
const getMyGroup = (params, callback) => {
  // 查找我的群
...
const getGroupUsers = (params, callback) => {
  // 查找指定群聊成员
...
const huntGroups = (params, callback) => {
  ...
  groupUser.findGroupByUserName(params.userName, (err, res) => {
    if (err) {
      ...
      let arr = []
      params.type === '2'
        ? (arr = [{ title: { $regex: key, $options: '$i' } }])
        : (arr = [{ code: { $regex: key, $options: '$i' } }])
      groups.count(
        {
          $or: arr,
...
const InsertGroupUsers = (params, callback) => {
  ...
  groupUser
    .find({ groupId: params.groupId, userId: params.userM })
    .then(res => {
      if (!res.length) {
        groupUser.create(val).then(r => {
          if (r['_id']) {
            groups
              .update({ _id: params.groupId }, { $inc: { userNum: 1 } })
              .then(raw => {
                if (raw.nModified > 0) {
                  callback({ code: 0 })
                } else {
