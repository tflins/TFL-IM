...
const getTodoList = (params, callback) => {
  // 查找我的日程
  todo
    .find(params)
    .then(r => {
...
const upTodo = (params, callback) => {
  // 编辑日程
  todo
    .update({ _id: params['_id'] }, params)
    .then(raw => {
      if (raw.nModified > 0) {
        callback({ code: 0 })
      } else {
