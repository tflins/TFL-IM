...
friendlySchema.statics = {
  findFriendByUserM: function(userId, callback) {
    // 通过userM查找userY信息
    return this.find({ userM: userId })
      .populate({ path: 'userY', select: 'signature photo nickname' }) // 关联查询
      .exec(callback)
  },
  findFriendByUserY: function(userId, callback) {
    // 通过userM查找userY信息
    return this.find({ userY: userId })
      .populate({ path: 'userM', select: 'signature photo nickname' }) // 关联查询
      .exec(callback)
  }
}
...
const findMyfriends = (params, callback) => {
  // 查找我的好友
...
const checkMyfriends = (params, callback) => {
  // 验证是否已加为好友
