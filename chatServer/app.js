...
const bodyParser = require('body-parser') // post请求需要的中间件
...
app.use('/v*', (req, res, next) => {
  if (req.session.login) {
    ...
    if (
      req.originalUrl === '/v/user/login' ||
      req.originalUrl === '/v/user/signUp'
    ) {
      next()
    } else {
      res.json({
      ...
      apiList.addFriend(val, r => {
        if (r.code === 0) {
          ...
          apiList.ServeraddConversitionList(val.name, userYparams, () => {
            apiList.ServeraddConversitionList(
              val.userYloginName,
              userMparams,
              () => {
                socket.to(value.roomid).emit('takeValidate', value)
